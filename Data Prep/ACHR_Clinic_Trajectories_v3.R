...
graph_trajectory  <- function(e, bucket_CFs, cf, n_gram_size=2, reference_day=1, filter_threshold=0,keep_ngram_vectors=FALSE, save_file_name='deleteme') {
  ...
  if  (!(bucket_col  %in% colnames(e)))  {  e = combineContextFactors(e,bucket_CFs,bucket_col) }
  ...
  for (b in blist){
    ...
    if (nrow(th)>3) 
      {
      ngdf = count_ngrams(th, 'threadNum', cf, n_gram_size)[1:2]    
  ...
  df = data.table(
    ...
    # Role_ymd = character(nWindows),
    # Role = character(nWindows),
  ...
  for (i in 1:(nWindows-1) ) {
    ...
    df[i,  Clinic := unlist(strsplit(blist[i],'_',fixed=TRUE))[1] ]
    df[i,  ymd :=    unlist(strsplit(blist[i],'_',fixed=TRUE))[2] ]

    # df[i,  Role_ymd := as.character(blist[i]) ]
    # df[i,  Role := unlist(strsplit(as.character(blist[i]),'+',fixed=TRUE))[1] ]
    # df[i,  ymd :=  unlist(strsplit(as.character(blist[i]),'+',fixed=TRUE))[2] ]
...
graph_trajectory_ROLES  <- function(e, bucket_CFs, cf, n_gram_size=2, reference_day=1, filter_threshold=0,keep_ngram_vectors=FALSE, save_file_name='deleteme') {
